...
The committed output folders are curated sample runs produced by the current
notebook:
...
They cover a small tree case, a larger bicyclic member with recursive branch
depth, and a larger `r=3` bicyclic member.  Older output folders generated by
earlier drawing code have been removed from the tracked repository.
...
- `*_compressed_schematic.png/pdf`: construction-compressed drawing of the
  current graph.  It follows the mathematical construction
  `C_k -> S_{r,k}^{(0)} -> S_{r,k,h} -> G_{r,k,h}=F_r(S_{r,k,h})` instead of
  trying to draw every repeated pendant tree.
...
- `*_family_h_variable.png/pdf`: schematic for the infinite family with fixed
  `r,k` and varying `h`, highlighting the recursive branch
  `R_0, R_1, ..., R_h`.
- `*.dot`, `*.gexf`, and `*_sfdp.svg`: Graphviz and Gephi exchange files.

The default drawing for large graphs is the construction-compressed schematic.
Exact full-graph DOT/GEXF files are still written for inspection in Graphviz or
Gephi, but Graphviz `sfdp` rendering is disabled by default because it can run
for a long time on graphs with thousands of vertices and does not produce a
paper-style figure.  The notebook prints a proper straight-edge crossing count
for the compressed schematic and for structural drawings that are generated.
